...
- **File Upload System**: "Use case definition" tab in CRUD modal supporting PDF (default), PowerPoint (.pptx, .ppt), and images (.jpg, .png, .gif, .bmp, .webp) with automatic PDF conversion for consistent preview. Uses object storage with LibreOffice for PowerPoint conversion and ImageMagick for image conversion. Includes backend proxy endpoint (`/api/proxy/presentation/:encodedUrl`) for secure file serving that resolves Google Cloud Storage direct access issues.
