...
- **Sequential Governance Gate UI (Latest)**: Updated GovernanceStepperLegoBlock to show "Waiting" status with lock icons for gates blocked by prerequisites. Intake gate waits for Operating Model; RAI gate waits for Intake. Tooltips explain which prior gate is blocking. Added "Reference Library Analytics" badges to all 4 Insights tabs.
