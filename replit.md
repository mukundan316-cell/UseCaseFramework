...
- **Governance & UX Simplification (Latest)**: Simplified Operating Model gate to require only Primary Business Owner (removed Business Function and status requirements). New use cases now default to "Discovery" status automatically. Restructured CRUD modal from 5 cluttered tabs into 4 intuitive tabs: Essentials, Scoring & Value, Responsible AI (new dedicated tab with progress badge), and Details (merged content).
