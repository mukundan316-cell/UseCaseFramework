  ...
  useEffect(() => {
    if (mode === 'edit' && useCase) {
      ...
      const formData = {
        ...
        valueValidator: (useCase as any).valueValidator || '',
        ...
        capabilityVendorFte: (useCase as any).capabilityTransition?.staffing?.current?.vendor?.total ?? 0,
        capabilityClientFte: (useCase as any).capabilityTransition?.staffing?.current?.client?.total ?? 0,
        capabilityIndependence: (useCase as any).capabilityTransition?.independencePercentage ?? 0,
  ...
  const onSubmit = async (data: FormData) => {
    try {
      const sanitizedData = {
        ...
        validationResponsibility: (data as any).validationResponsibility || '',
